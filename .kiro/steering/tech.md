...
- **The four defaults, and the fact that no config file is required.** When a value is undeclared it takes its default: scope `@microservices`; microservice root `packages/microservices`; common root `packages/common`; spa root `packages/spa`. A project with no `scaffold.config.json` at all is valid and takes all four defaults — which is exactly how this repository is configured — and `{}` behaves identically to an absent file. These four defaults are declared as literals in exactly one module (`packages/build-tools/src/project-config.ts`); no other Build_System source spells the scope or a root of its own.
- **Load once, thread everywhere.** The Build_System reads the Project_Config_File at most once per run and threads the single resulting Effective_Config through every component — package discovery, the Repo_Invariant_Checker, the registry generator, the workspace build-order and Project_List derivations, and the image-tree assembler. No component reads a scope or a discovery-root literal of its own, so no two components of one run can operate on different configurations.
...
- **A rejected config fails the run before any discovery or build.** If the Project_Config_File cannot be read, is not JSON, is not an object, declares a wrong-typed or unknown key, or declares an invalid scope or root path, the run terminates with a non-zero status before any package is discovered, no registry is written, no build order is derived, and no image tree is assembled — leaving any already-present generated `Dockerfile` and Microservice_Registry unchanged. It reports one diagnostic per rejected value, naming the JSON key path (or category), the offending value, and the reason it was rejected.

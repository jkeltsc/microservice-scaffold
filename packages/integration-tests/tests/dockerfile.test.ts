// Structural assertions on the repo-root Dockerfile.
//
// A real `docker build` is far too slow for this suite, but the Dockerfile's
...
// parts that matter — most importantly that the runtime payload is the single
// staged `/out` copy and that nothing copies `packages/microservices` into the
// image, which is precisely the way a "specific" image ends up carrying every
// microservice anyway.
//
...
describe("root Dockerfile structure", () => {
  ...
  it("defines exactly the build and runtime stages", () => {
    ...
    expect(stages).toEqual(["build", "runtime"]);
  });

  it("takes its whole runtime payload from a single copy of the staged tree", () => {
    ...
    expect(runtimeCopies).toEqual(["COPY --from=build /out ./"]);
  });
  ...
  it("uses BUILDPLATFORM for the build stage", () => {

...
//   - a REAL Microservice_Registry naming all three microservices, generated by
//     the generator bin into the Entry_Package's tree. Since the registry
//     inversion no install step installs a registry at all — the Template_Copy_
//     Step and the Registry_Template are both gone — so generating it explicitly
//     is the only way the registry exists here.
//
// Pre-fix, the derived order placed the statically importing package ahead of
// `packages/microservices/microservice1`, so compiling it against that real
// registry failed with
...
// microservice (statement 4) before the Overseer (statement 5) and the
// Entry_Package — the registry's only static importer since the inversion — after
// both (statement 6), so the registry's static imports resolve and the ordered
// build exits 0 (2.8; registry-inversion R8.1–R8.3).
//
...
/** The unconfigured ProjectContext of this repository (and of the pristine copy,
 *  which carries no `scaffold.config.json` either). */
const cleanTreeContext = projectContext(defaultEffectiveConfig());
/**
 * The generated (real) registry the generator writes for the Selector. Since the
 * registry inversion it lives in the CONSUMER's tree under the Entry_Root, and the
 * path comes from the single derivation rather than being spelled here
 * (registry-inversion R4.1, R5.8).
 */
const REGISTRY_REL = generatedRegistryPath(cleanTreeContext);
/**
 * The Entry_Package's compiled entrypoint — the module the whole ordered build
 * exists to produce, present iff its `tsc` pass succeeded. It is the
 * Entry_Point_Path, read off the context (registry-inversion R7.1).
 */
const ENTRY_POINT_REL = cleanTreeContext.entryPointPath;
/**
 * The compiled registry import — the module 1.3's TS2307 was about. It now
 * compiles as part of the Entry_Package rather than the Overseer, since the
 * Entry_Module is the only static importer of the Generated_Registry
 * (registry-inversion R2.5, R3.3).
 */
const ENTRY_REGISTRY_JS_REL = `${cleanTreeContext.entryRoot}/dist/generated/microservice-registry.js`;
...
describe("unified-build-order — clean-tree ordered build with a real registry compiles the Overseer (1.3, 1.4, 2.8, 2.21)", () => {
  ...
  it(
    ...
    () => {
      ...
      // microservices). `npm ci` installs no registry since the inversion retired
      // the Template_Copy_Step, so this generation is what puts one on disk. This
      ...
      expect(existsSync(resolve(treeDir, ENTRY_POINT_REL))).toBe(false);
      ...
      // And the Entry_Package's compiled output — including the compiled registry
      // import that 1.3's TS2307 named — exists afterward, which it cannot if the
      // Entry_Package's `tsc` pass failed. The Entry_Package is the statically
      // importing side of the registry since the inversion, and it builds after
      // every microservice and after the Overseer.
      expect(existsSync(resolve(treeDir, ENTRY_POINT_REL))).toBe(true);
      const entryRegistryJs = resolve(treeDir, ENTRY_REGISTRY_JS_REL);
      expect(existsSync(entryRegistryJs)).toBe(true);
      // A non-empty compiled artifact, not a stray empty file.
      expect(statSync(entryRegistryJs).size).toBeGreaterThan(0);
    },
    BUILD_TIMEOUT_MS,
  );
});
